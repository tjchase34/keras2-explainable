...
def explain_step(
    ...
    **params,
    ...
    inputs, indices, _ = tf.keras.utils.unpack_x_y_sample_weight(data)
    logits, maps = method(
        model=model,
        inputs=inputs,
        indices=indices,
...
#     all_outputs = tf.keras.tree.map_structure_up_to(
#         batch_outputs, tf.concat, outputs
#     )
